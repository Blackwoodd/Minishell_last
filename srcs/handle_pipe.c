...
/*   Updated: 2023/06/14 16:58:58 by nbechon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
	...
	*It first checks if the value of last_pipe_out is 0, indicating an
	invalid pipe file descriptor.
		...
		-If the condition is true, it writes an error message, "Error, opening pipe,"
		to the standard error file descriptor using ft_putstr_fd.
...
/*
	...
	*It uses the pipe system call to create a new pipe and store the file
	descriptors in the pipes array. If the pipe call fails (returns a non-zero
	value), indicating an error, it returns ft_perror(EXIT_FAILURE, "pipe error").
...
/*
	...
	*The function pipe_t_two takes two parameters: last_pipe_out, a pointer
	to the last pipe output file descriptor, and exp_tok, a pointer to a
	structure t_exp_tok containing command execution information.
	...
	*It checks if *last_pipe_out is equal to 0, which indicates an error
	opening the pipe.
	If true, it prints an error message and returns EXIT_FAILURE.
	
	*If exp_tok->in is 0, indicating that the input file descriptor has not
	been set yet,
	...
	*It uses the pipe system call to create a new pipe and store the file
	descriptors in the pipes array. If the pipe call fails
	(returns a non-zero value), it returns ft_perror(EXIT_FAILURE,"pipe error").
	...
	*Next, it checks if exp_tok->out is 1, indicating that the output
	file descriptor has not been set yet.
		...
		-If the condition is false, indicating that exp_tok->out is already
		set, it enters the else block.
...
/*
	...
	*The function then uses a series of conditional statements to handle
	different pipe_type values:
		...
		for handling cases where the last_pipe_out value is not a valid
		file descriptor, and it returns an appropriate exit status.
